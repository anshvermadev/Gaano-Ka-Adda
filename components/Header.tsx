...
export default function Header(): React.JSX.Element {
  ...
  const spotifyUrl = "https://open.spotify.com/search/90s%20hindi%20nostalgia%20hits";

  return (
    <header className="fixed top-0 left-0 right-0 z-40 px-6 sm:px-10 py-5 flex items-center justify-between pointer-events-none drop-shadow-md">
      ...
      >
        <div className="relative group/clock">
          <span className="text-white/90 text-sm font-medium tracking-wider font-mono drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] cursor-default">
            {timeString || '5:30 pm'}
          </span>
      ...
      >
        <div className="relative group/online">
          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/15 text-xs text-white/95 font-medium shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:border-white/30 transition-all cursor-default">
      ...
      {/* Right: Music Links with Custom Tooltips */}
      <motion.div 
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.1 }}
        className="flex items-center gap-4 justify-end pointer-events-auto"
      >
        {/* Spotify Link */}
